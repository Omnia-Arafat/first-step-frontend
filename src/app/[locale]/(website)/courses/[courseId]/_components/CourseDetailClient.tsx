...
import { bookCourseAction } from "@/actions/courseActions";
      ...
      <div className="container mx-auto px-4 pb-16 space-y-12">
        ...
        >
          <ul className="space-y-3">
            {course.requirements[locale].map((req, index) => (
        ...
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-primary-blue-50 rounded-xl p-6">
              ...
              <p className="text-primary-blue font-medium whitespace-pre-line">
            ...
            <div className="bg-primary-green-50 rounded-xl p-6">
              ...
              <p className="text-primary-green-700 font-medium">
                {course.price[locale]}
              </p>
            </div>
          </div>
...
function BookingForm({
  ...
  const onSubmit = (data: BookingFormData) => {
    startTransition(async () => {
      try {
        const result = await bookCourseAction({
          name: data.name,
          phone: data.phone,
          email: data.email,
          childrenNames: data.children.map((c) => c.name),
          courseName: course.name[locale],
        });
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              {form.formState.errors.phone && (
                <p className="text-sm text-destructive">
                  {t(`form.errors.${form.formState.errors.phone.message}`)}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-4">
                ...
                <div key={field.id} className="flex items-center gap-3">
                  ...
                  {fields.length > 1 && (
                    ...
                    >
                      ...
                      <span className="hidden sm:inline ltr:ml-1 rtl:mr-1">
                        {t("form.removeChild")}
                      </span>
                    </Button>
                  )}
            ...
            {form.formState.errors.children?.root && (
              <p className="text-sm text-destructive">
                {t(
                  `form.errors.${form.formState.errors.children.root.message}`
                )}
              </p>
            )}
          </div>
